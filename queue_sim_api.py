...
class Source(object):
    ...
    def run(self):
        ...
        while True:
            ...
            generated_packet = Packet(
                id=self.packet_count, size=packet_size, generation_timestamp=env.now)
            if self.debug:
    ...
    def run(self):
        while True:
            ...
            if self.count_bytes:
                ...
                total = len(self.queued_server.buffer.items) + \
                    self.queued_server.busy
            self.sizes.append(total)
            # Print average packets number and latency
            if self.count_bytes:
                average_packet_nb = np.mean(self.sizes)
                print("Average packets number: %d", average_packet_nb)
